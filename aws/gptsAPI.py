...
def submit_message(assistant_id, thread, user_message):
    ...
    client.beta.threads.messages.create(
        thread_id=thread,
        role="user",
        content=user_message + "time:" + str(now),
    )
...
def filter_response(response):
    ...
    filtered_response = re.sub(r"[^a-zA-Z가-힣ㄱ-ㅎㅏ-ㅣ\s.,!?()\-0123456789]", "", response)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        ASSISTANT_ID = response_json['assistant_id']
        THREAD_ID = response_json['thread_id']
        logger.info(f"ID 가져오기 성공: {ASSISTANT_ID}, {THREAD_ID}")
    ...
    try:
        ...
        response = requests.post(url, data=json.dumps(data), headers=headers)
        logger.info(f"DB 저장 완료: {response}")
    except Exception as e:
        return {
            'statusCode': 500,
            'body': '대화 내용 저장 오류'

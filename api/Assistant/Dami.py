...
# 고정된 스레드 ID를 설정(스레드가 바뀌면 이전 대화내용 리셋)
FIXED_THREAD_ID = "thread_H2skFHhzHVfjAMGGYVa2MSfS"
...
def print_messages(thread_id):
    ...
    assistant_message_printed = False  # 플래그를 추가하여 첫 번째 assistant 메시지만 출력하도록 설정
    ...
    for msg in messages:
        ...
        elif msg.role == "assistant" and not assistant_message_printed:
            content_text = msg.content[0].text.value if msg.content else "No content"
            print(f"assistant: {content_text}")
            assistant_message_printed = True  # 첫 번째 assistant 메시지 출력 후 플래그를 설정

        # 두 메시지가 모두 출력되면 루프를 종료
        if user_message_printed and assistant_message_printed:
            break

# 사용자 메시지
user_message = send_message(FIXED_THREAD_ID, "user", "안녕! 뭐하고 지내?") # 사용자 메시지 입력
...
run = run_assistant(FIXED_THREAD_ID, "asst_DoU3DINXOpUaNyNkbn0fmwnW", instructions)

if run.status == 'completed': 
    print_messages(FIXED_THREAD_ID)
else:
    print(run.status)
